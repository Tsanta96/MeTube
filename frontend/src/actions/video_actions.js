...
export const createVideo = (video, config) => dispatch =>
  VideoApiUtil.createVideo(video, config).then(video =>
    dispatch(receiveVideo(video))
  );

export const deleteVideo = videoId => dispatch =>
  VideoApiUtil.deleteVideo(videoId).then(() => dispatch(deleteVideo(videoId)));

export const fetchSearchVideos = search => dispatch => {
  return VideoApiUtil.fetchSearchVideos(search).then(search => {
    // debugger;
...
export const fetchTrendingVideos = timeSpan => dispatch => {
  return VideoApiUtil.fetchTrendingVideos(timeSpan).then(videos =>
    dispatch(receiveTrendingVideos(videos))
  );
};

export const fetchUserUploads = id => dispatch =>
  VideoApiUtil.getUserUploads(id).then(videos =>
    dispatch(receiveUserUploads(videos))
  );
